...
const tokenSlice = createSlice({
  ...
  reducers: {
    ...
    setActiveTab: (state, action: PayloadAction<"new-pairs" | "final-stretch" | "migrated">) => {
    ...
    updatePrice: (state, action: PayloadAction<{ id: string; price: number; previousPrice: number }>) => {
      ...
      const filteredToken = state.filteredTokens.find((t) => t.id === action.payload.id);
      if (filteredToken) {
